...
function UploadWorkspace() {
  ...
  const ytPreviewFallbackThumb = useMemo(() => {
    const id = extractYoutubeVideoId(normalizeYoutubeUrl(url));
  ...
  async function fetchYoutubePreview(targetUrl: string) {
    ...
    try {
      ...
      setYtPreview(null);
      if (/^not found$/i.test(msg) || isLikelyYoutubeSourceBlock(msg)) {
        setYtPreviewError("Preview unavailable for this link.");
      } else {
  ...
  // YouTube URL changes: keep preview state synced (debounced preview call)
  useEffect(() => {
      ...
      if (isInsufficientCreditsError(e)) {
        ...
        fail(
          ...
          [
            ...
            "Upload an MP4 in the left card to continue.",
            "",
            `Details: ${msg}`,
          ].join("\n")
        );
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-[12px] text-white/55">
                ...
                <div className="mt-3 text-white/45">
                  If direct import is blocked, upload an MP4 in the left panel.
                </div>
              </div>

              <div className="pt-1 text-[12px] text-white/35">
                Tip: if direct import is blocked, upload the file manually on the left.
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="surface-soft relative overflow-hidden p-6">
            ...
            <div className="relative">
              ...
              <div className="mt-3 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-[12px] text-white/65">
                If blocked: Orbito shows a source-blocked error. Then upload MP4 manually.
              </div>
