...
function LoginRedirectInner() {
  ...
  const [status, setStatus] = useState("Redirecting to Orbito login...");
  ...
  const bridgeToken = useMemo(
    () => (searchParams?.get("bridge_token") || "").trim(),
    [searchParams]
  );
    ...
    async function run() {
      if (bridgeToken) {
        try {
          ...
          if (cancelled) return;
          const target = withBasePath(sanitizeNextPath(result?.next || nextPath));
      ...
      window.location.replace(orbitoLoginUrl(nextPath));
    }
