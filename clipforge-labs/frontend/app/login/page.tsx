...
function LoginPageInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mounted) {
          // Always show the social options (even if not configured yet).
  ...
  async function onSocial(provider: Provider) {
    ...
    if (!oauthConfigured[provider]) {
      setSocialError(`${providerLabel(provider)} sign-in isn’t available right now. Please use email.`);
      return;
    }
            ...
            <div className="mt-6 grid gap-8 lg:grid-cols-12 lg:gap-10">
              ...
              <div className="lg:col-span-5">
                ...
                <p className="mt-3 max-w-md text-sm leading-relaxed text-white/65 md:text-[15px]">
                  Sign in to generate videos, manage jobs, and clips.
                </p>
                {checkSlow && (
                    ...
                    <div className="mt-5 grid gap-2">
                      {oauthProviders.map((p) => (
                        <ProviderButton
                          ...
                          provider={p}
                          onClick={() => onSocial(p)}
                          disabled={!!socialBusy || submitting || !oauthConfigured[p]}
                        />
                      ))}
                    </div>
