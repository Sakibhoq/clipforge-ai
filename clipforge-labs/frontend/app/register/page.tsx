...
/* =========================================================
   ...
   - Social buttons now redirect to OAuth providers (when configured)
========================================================= */
...
const AUTH_PROVIDER_ORDER: Provider[] = ["google", "facebook", "tiktok"];
...
function RegisterPageInner() {
  ...
  const [oauthProviders, setOauthProviders] = useState<Provider[]>(AUTH_PROVIDER_ORDER);
  const [oauthConfigured, setOauthConfigured] = useState<Record<Provider, boolean>>({
    google: true,
    facebook: false,
    tiktok: false,
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const nextConfigured: Record<Provider, boolean> = {
          google: false,
          facebook: false,
          tiktok: false,
        };
        ...
        for (const r of rows) {
          const key = r.provider as Provider;
          if (!AUTH_PROVIDER_ORDER.includes(key)) continue;
          enabled.add(key);
          nextConfigured[key] = !!r.configured;
        }

        if (mounted) {
          setOauthProviders(AUTH_PROVIDER_ORDER.filter((p) => enabled.size === 0 || enabled.has(p)));
          setOauthConfigured(nextConfigured);
        }
      } catch {
  ...
  async function onSocial(provider: Provider) {
    ...
    if (!oauthConfigured[provider]) {
      setSocialError(`${providerLabel(provider)} sign-up isn’t available right now. Please use email.`);
      return;
    }
                      ...
                      <div className="mt-4 grid gap-2">
                        {oauthProviders.map((p) => (
                          <ProviderButton
                            ...
                            provider={p}
                            onClick={() => onSocial(p)}
                            disabled={submitting || !!socialBusy || !oauthConfigured[p]}
                          />
                        ))}

                        {socialBusy && (
                          <div className="mt-2 flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.06] px-4 py-3 text-[12px] text-white/70">
                            <Spinner />
                      ...
                      <form onSubmit={onEmailSubmit} className="mt-4 grid gap-4">
                        ...
                        >
                          <span
                            ...
                            style={{
                              background:
                                "radial-gradient(140px 90px at 35% 45%, rgba(255,183,3,0.28), transparent 70%), radial-gradient(140px 90px at 70% 55%, rgba(58,134,255,0.20), transparent 72%)",
                            }}
                          />
