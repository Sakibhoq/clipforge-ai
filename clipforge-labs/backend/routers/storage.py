...
# Proxy-chunk fallback default.
# Larger chunks dramatically reduce request overhead for big files.
UPLOAD_PROXY_CHUNK_SIZE = int(os.getenv("UPLOAD_PROXY_CHUNK_SIZE", str(5 * 1024 * 1024)))  # 5MB
...
def presign_put(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Object storage credentials not available (set AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY for S3-compatible storage).",
        )
...
async def upload_proxy(
    ...
    Works with both STORAGE_BACKEND=local and STORAGE_BACKEND=s3/gcs.
    """
    if not file:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Object storage credentials missing for upload",
        )
    except ClientError as e:
        raise HTTPException(status_code=500, detail=str(e))
    except Exception as e:
